...
async function testNewFalleroYear() {
  try {
    ...
    // Show all ordinary dates with status
    console.log('\n=== TODOS LOS ORDINARIOS CON ESTADO ===');
    allDates?.filter(d => d.lottery_type === 'ordinary').forEach((date, index) => {
      const isInNewFallero = getLotteryDatesByType([date], 2026, 'ordinary').length > 0;
      console.log(`${index + 1}. ${date.date} - ${date.lottery_type} - Fallero 2026: ${isInNewFallero ? '✅' : '❌'}`);
    });
